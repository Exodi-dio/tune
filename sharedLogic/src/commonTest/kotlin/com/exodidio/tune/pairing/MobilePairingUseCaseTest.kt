...
class MobilePairingUseCaseTest {
    ...
    fun parsesDesktopNameAndFallsBackForLegacyQr() {
        ...
        val named = PairingQrParser.parse("tune://pair/v1?host=192.168.1.8&port=1883&desktop_id=01234567-89ab-cdef-0123-456789abcdef&public_key=$key&desktop_name=Living%20Room")
        assertEquals("Living Room", named.getOrThrow().displayName)
        val legacy = PairingQrParser.parse("tune://pair/v1?host=192.168.1.8&port=1883&desktop_id=01234567-89ab-cdef-0123-456789abcdef&public_key=$key")
        assertEquals("Tune Desktop", legacy.getOrThrow().displayName)
    }
    ...
    fun requestSignsAndPublishesToTheV1Topics() = runTest {
        ...
        val result = useCase.pair("tune://pair/v1?host=10.0.0.2&port=1234&desktop_id=01234567-89ab-cdef-0123-456789abcdef&public_key=$key")

        assertIs<PairingResult.Failed>(result)
        assertEquals(PairingFailure.TimedOut, result.failure)
        assertEquals("tune/pairing/v1/01234567-89ab-cdef-0123-456789abcdef/response/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa", transport.responseTopic)
        assertEquals("tune/pairing/v1/01234567-89ab-cdef-0123-456789abcdef/request", transport.requestTopic)
