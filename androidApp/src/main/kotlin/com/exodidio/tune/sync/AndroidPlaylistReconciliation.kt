...
import com.exodidio.tune.sync.PlaylistMutationResult
import com.exodidio.tune.sync.PlaylistMutationBatch
import com.exodidio.tune.sync.PlaylistMutationBatchResult
import com.exodidio.tune.sync.PlaylistReconciliationRequest
import com.exodidio.tune.sync.PlaylistReconciliationTransport
import com.exodidio.tune.sync.PlaylistArtworkStagingStore
import com.exodidio.tune.sync.ListeningSyncSnapshot
import com.exodidio.tune.sync.ListeningSyncStore
import com.exodidio.tune.sync.ListeningSyncProtocol
...
internal class AndroidPlaylistReconciliationTransport(
    ...
    private suspend fun request(method: String, rawUrl: String, mobileId: String, body: ByteArray, contentType: String): PlaylistHttpResponse {
        ...
        val connection = (uri.toURL().openConnection() as HttpURLConnection).apply {
            ...
            setRequestProperty("X-Tune-Mobile-ID", mobile.id)
            setRequestProperty("X-Tune-Timestamp", timestamp)
            setRequestProperty("X-Tune-Nonce", nonce)
            setRequestProperty("X-Tune-Signature", signature)
        }
        return try {
...
internal fun readStagedPlaylistArtwork(filesDir: File, staged: com.exodidio.tune.sync.StagedPlaylistArtwork, expectedHash: String): ByteArray {
